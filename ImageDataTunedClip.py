"""
CLIP-based Hate Speech Classifier - IMAGE ONLY
Uses only image embeddings from CLIP for classification
"""
...
MODEL_SAVE_PATH = 'best_clip_image_only_classifier.safetensors' 
...
# Step 2: Create Dataset (IMAGE ONLY)
...
# Step 3: Define Model (IMAGE ONLY)
class CLIPImageOnlyClassifier(nn.Module):
    def __init__(self, clip_model_name="openai/clip-vit-large-patch14", unfreeze_layers=2):
        ...
        # Unfreeze last N layers of vision encoder only
        ...
        # Classification head (only image embeddings)
...
train_dataset = CLIPImageOnlyDataset(X_train, y_train, processor)
val_dataset = CLIPImageOnlyDataset(X_val, y_val, processor)
test_dataset = CLIPImageOnlyDataset(X_test, y_test, processor)
...
print("\nInitializing CLIP-based IMAGE ONLY classifier...")
model = CLIPImageOnlyClassifier(unfreeze_layers=4) 
...
print("\n" + "="*80)
print("Starting Training - IMAGE ONLY MODEL")
print("="*80)
...
plt.title('Loss History - Image Only')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.grid(True)
...
plt.title('Accuracy History - Image Only')
plt.xlabel('Epoch')
plt.ylabel('Accuracy (%)')
plt.legend()
plt.grid(True)
...
plt.tight_layout()
plt.savefig('images/accuracyCLIP_image_only.png', dpi=300, bbox_inches='tight')
...
plt.savefig('images/confusionMatrixClip_image_only.png', dpi=300, bbox_inches='tight')

print("\nImage-only model training finished.")